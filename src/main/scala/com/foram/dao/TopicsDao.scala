...
object TopicsDao extends BaseDao {
  ...
  def findByUserID(user_id: UUID): Future[Seq[Topic]] = db.run(topics.filter(_.userID === user_id).result)

  def findByCategoryID(category_id: UUID): Future[Seq[Topic]] = db.run(topics.filter(_.categoryID === category_id).result)
