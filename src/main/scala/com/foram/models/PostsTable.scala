...
class PostsTable(tag: Tag) extends Table[Post](tag, "posts") {
  ...
  def user = foreignKey("post_user_fk", userID, TableQuery[UsersTable])(_.id, onUpdate = ForeignKeyAction.Cascade, onDelete = ForeignKeyAction.Cascade)

  def topic = foreignKey("post_topic_fk", topicID, TableQuery[TopicsTable])(_.id, onUpdate = ForeignKeyAction.Cascade, onDelete = ForeignKeyAction.Cascade)

  def topicSlugFK = foreignKey("post_topic_slug_fk", topicSlug, TableQuery[TopicsTable])(_.slug, onUpdate = ForeignKeyAction.Cascade, onDelete = ForeignKeyAction.Cascade)
